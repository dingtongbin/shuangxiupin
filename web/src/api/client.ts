...
export const http = axios.create({
  // 部署到 GitHub Pages 等静态托管时，通过 VITE_API_BASE 指向自建后端（如 https://api.example.com）
...
export async function call<T>(fn: () => Promise<{ data: ApiBody<T> }>): Promise<T | null> {
  try {
    ...
    if (e instanceof ApiError) {
      Toast.show({ content: e.message, position: "bottom" });
    } else {
      Toast.show({ content: "网络异常，请稍后再试", position: "bottom" });
    }
    return null;
  }
}

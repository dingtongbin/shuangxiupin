// 系统管理控制台：独立服务进程（默认 :8090），仅内网访问。
// 与主服务共享同一套 internal/ 业务代码、MySQL 与 Redis（会话互通），
// 但只暴露管理员登录/会话与 /api/v1/sys 管理接口，不含任何用户端/运营端能力。
package main
...
func main() {
	cfgPath := flag.String("config", "configs/admin.yaml", "配置文件路径")
	flag.Parse()
	...
	r := router.NewAdmin(h, sm, cfg, rbac)

	srv := &http.Server{Addr: cfg.Server.Addr, Handler: r}
	go func() {
		if err := srv.ListenAndServe(); err != nil && err != http.ErrServerClosed {
			slog.Error("系统管理服务异常退出", "err", err)
			os.Exit(1)
		}
	}()
	slog.Info("系统管理控制台已启动（仅内网）", "addr", cfg.Server.Addr, "mode", cfg.Server.Mode)
